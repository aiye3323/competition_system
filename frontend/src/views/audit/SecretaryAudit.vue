...
import { getPendingList, secretaryApprove, secretaryReject, getAuditLogs } from '@/api/audit'
...
const submittingAction = ref('') // 'approve' or 'reject'
...
async function fetchData() {
  loading.value = true
  try {
    const params = { role: 'SECRETARY', page: page.value, size: size.value }
...
async function handleApprove() {
  ...
  try {
    const res = await secretaryApprove(currentDetail.value.id, { opinion: form.opinion })
    if (res.code === 200) {
...
async function handleReject() {
  ...
  submitting.value = true
  try {
    const res = await secretaryReject(currentDetail.value.id, { opinion: form.opinion })
    if (res.code === 200) {
